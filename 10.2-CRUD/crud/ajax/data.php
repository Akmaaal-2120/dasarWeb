<table id="example" class="table table-striped table-bordered" style="width:100%">
    ...
    <tbody>
        ...
        // Asumsi $db1 sudah didefinisikan di koneksi.php sebagai objek PDO
        ...
        if (count($res1) > 0) {
            foreach ($res1 as $row) {
                $id = $row['id'];
                $nama = $row['nama'];
                // Logika ternary untuk menampilkan Jenis Kelamin: L = Laki-laki, P = Perempuan
...
<script type="text/javascript">
    ...
    // 1. Ambil ID dari tombol yang diklik
    var id = $(this).attr('id'); 
    
    // 2. Panggil AJAX
    $.ajax({
        type: 'POST',
        url: "hapus_data.php", // Endpoint PHP untuk menghapus
        data: {
            id: id // Kirim ID yang akan dihapus
        },
        success: function() {
            // 3. Setelah sukses, muat ulang tabel data
            $('.data').load("data.php"); 
        },
        error: function(response) {
            console.log(response.responseText);
